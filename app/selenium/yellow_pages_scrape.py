...
def scrape_property_managers(location=""):
    driver = setup_driver()
    search_url = f"https://www.yellowpages.com/search?search_terms=property+managers&geo_location_terms={location.replace(' ', '+')}"
        ...
        for listing in listings:
            ...
            if name:  # Only add if we have at least a name
                all_results.append({
                    ...
                    "type": "property_manager"  # Mark as property manager
                })
...
def scrape_all_cities():
    """Scrape property managers for all defined cities"""
    total_results = []
    
    for city in CITIES:
        print(f"\n🛠 Scraping property managers in {city}...")
        print("=" * 50)
        
        try:
            scraped_data = scrape_property_managers(city)
            if scraped_data:
                print(f"📊 Found {len(scraped_data)} property managers in {city}")
...
if __name__ == "__main__":
    print("🚀 Starting Yellow Pages Property Manager Scraper")
